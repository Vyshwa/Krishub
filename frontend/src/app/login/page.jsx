...
function LoginContent() {
  ...
  useEffect(() => {
    if (login.isSuccess) {
      if (redirect) {
        router.push(`/sso?redirect=${encodeURIComponent(redirect)}`);
      } else {
        router.push('/');
      }
    }
  }, [login.isSuccess, router, redirect]);
      ...
      <div className="hidden md:flex md:w-1/2 bg-gradient-to-br from-primary/10 via-background to-secondary/20 p-12 flex-col items-center justify-center relative overflow-hidden border-r">
        ...
        <div className="relative z-10 flex flex-col items-center gap-8">
          ...
          <div className="text-center space-y-3">
            <h2 className="text-3xl font-bold tracking-tight text-primary text-balance">Empowering Your IT Infrastructure</h2>
